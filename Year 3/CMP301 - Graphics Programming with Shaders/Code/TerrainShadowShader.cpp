...
TerrainShadowShader::TerrainShadowShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"terrainShadow_vs.cso", L"shadow_ps.cso");
}
...
void TerrainShadowShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
...
void TerrainShadowShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texturePS, ID3D11ShaderResourceView* textureVS, ID3D11ShaderResourceView* depthMap, Light* light, float amplitude)
{
	...
	deviceContext->PSSetShaderResources(0, 1, &texturePS);

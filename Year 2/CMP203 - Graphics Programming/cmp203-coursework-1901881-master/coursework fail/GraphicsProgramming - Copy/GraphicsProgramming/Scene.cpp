...
Scene::Scene(Input *in)
{
	...
	//shape.loadTexture("gfx/skybox.png");
	//Camera
	...
	//Model
	model = new Model();
	//model->load("models/minecraftTree.obj", "gfx/logsSide.png");
	//model->load("models/minecraftPig.obj","");
	//shape = new Shape();
	//shape.loadTexture("gfx/stone_slab.png");
	//shape.textureLoad();
	...
	//Box
	//shape.proceduralBoxCreateNew(3,3,3);
	//shape.proceduralBoxCreate(1,1,1);

	textureImposter = SOIL_load_OGL_texture(
		"gfx/imposterTable.png",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
...
void Scene::render() {
	...
	//fenceRender();
	//model->render();
	//renderBathroomMirror();
	

	
	planesRender();//render last for blending
	doorRender();
	windowRender();
	// End render geometry --------------------------------------
...
void Scene::planesRender()
{
	...
	glPushMatrix();
	//texture->textureChoose(6);
	glTranslatef(-1, 0, -7);
	plane1.planeRender();
	glPushMatrix();
	//texture->textureChoose(7);
	glTranslatef(10, 0, 0);
	plane2.planeRender();
	glPopMatrix();
	glPopMatrix();
...
void Scene::createBathroomMirror()
{
	glPushMatrix();
	glTranslatef(44.9,0.5,-1);
	glBegin(GL_QUADS);
...
void Scene::renderBathroomMirror()
{
	//setting up stencil
	...
	//reenbling stuf to draw geometry
	glEnable(GL_DEPTH_TEST);
	glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);
	glStencilFunc(GL_EQUAL, 1, 1);
	glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP);

	//building mirrored universe
	...
	//reversing mirror stuff
	glDisable(GL_STENCIL_TEST);
	glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

	//drawing actual floor
	glDisable(GL_LIGHTING);//100 percent reflective serface
...
void Scene::tableCreate()
{
	...
	glTexCoord2f(0.0f, 0.0f);//starts mapping the texture onto the quad
...
void Scene::fenceCreate(float fenceNum)
{	
	for (int x = 0; x < fenceNum; x++)
	{
		glTranslatef(2.5, 0, 0);
		glPushMatrix();
		glScalef(0.5, 1, 0.5);
		fenceBase.proceduralBoxRender();
		glPopMatrix();
		glPushMatrix();
		glScalef(1, 0.3, 0.3);
		glTranslatef(0.5, 1.75, 0.25);
		fenceSide.proceduralBoxRender();
		glPushMatrix();
	...
	//fix this
	/*
	maybe do it by adding a number for every loop
	so one fence equals 
	fence base = 0.5 x
	fence side = 2 x
	fence = 2.5 x
	*/
	glScalef(0.5, 1, 0.5);
	glTranslatef(((fenceNum*2.5)+ 2), 0, 0);
	fenceBase.proceduralBoxRender();
	glPopMatrix();
}

void Scene::fenceRender()
{
	fenceCreate(3);
}

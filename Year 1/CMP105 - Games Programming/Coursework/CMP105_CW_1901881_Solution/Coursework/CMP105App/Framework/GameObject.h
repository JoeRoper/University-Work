...
public:
	...
	//Invis frames used to make sure the player or enemy dont lose all their health at once
	bool getInvis();
	void addInvis(float x) { invis += x; };
	void subtractInvis(float x);

	bool getInBoundaries() { return inBoundaries; };
	void setInBoundaries(bool x) { inBoundaries = x; }

	void setCurrentRoom(int x) { currentRoom = x; };
